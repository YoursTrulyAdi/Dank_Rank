...
app.post("/auth/token", async (req, res) => {
    ...
    try {
        ...
        const backendJWT = jwt.sign({ uid: decodedToken.uid, email: decodedToken.email }, JWT_SECRET, { expiresIn: "1h" });
...
app.post("/vote", authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        await memeRef.update({
            voteCount: voteType === "upvote" ? FieldValue.increment(1) : FieldValue.increment(-1)
        });
